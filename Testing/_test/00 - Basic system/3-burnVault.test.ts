...
describe("burnVault", function () {
  ...
  it("Basic burn and distribution with multiple wallets", async function () {
    ...
    await expect(
      burnVault.connect(addr2).backingWithdraw(10)
    ).to.changeTokenBalance(btcb, addr2.address, 10);
    await expect(
      burnVault.connect(addr3).backingWithdraw(20)
    ).to.changeTokenBalance(btcb, addr3.address, 20);
    await expect(
      burnVault.connect(addr4).backingWithdraw(20)
    ).to.changeTokenBalance(btcb, addr4.address, 20);
    await expect(
      burnVault.connect(addr5).backingWithdraw(40)
    ).to.changeTokenBalance(btcb, addr5.address, 40);
  });
});
